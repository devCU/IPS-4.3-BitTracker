...
\IPS\Content\MetaData
{
	...
	public function downloadCheck( array $record = NULL, \IPS\Member $member = NULL )
	{
		...
		{
			if ( \IPS\Db::i()->select( 'COUNT(*)', 'bitracker_sessions', array( array( 'dsess_start > ?', time() - ( 60 * 15 ) ), $member->member_id ? array( 'dsess_mid=?', $member->member_id ) : array( 'dsess_ip=?', \IPS\Request::i()->ipAddress() ) ) )->first() >= $restrictions['limit_sim'] )
			{
