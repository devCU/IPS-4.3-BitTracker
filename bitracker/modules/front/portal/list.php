 ...
 *
 * @brief       BitTracker List Files Controller
 ...
 */
class _list extends \IPS\Dispatcher\Controller
{
	...
	protected function _index()
	{
		...
		{
			\IPS\Output::i()->rssFeeds['bit_rss_title'] = \IPS\Http\Url::internal( 'app=bitracker&module=bitracker&controller=browse&do=rss', 'front', 'bitracker_rss' );
		}
	...
	protected function _category( $category )
	{
		...
		else
		{
			...
			$table->sortOptions = array_merge( $table->sortOptions, array( 'file_bitracker' => 'file_bitracker' ) );
	...
	protected function rss()
	{
		...
		$document = \IPS\Xml\Rss::newDocument( \IPS\Http\Url::internal( 'app=bitracker&module=bitracker&controller=browse', 'front', 'bitracker' ), \IPS\Member::loggedIn()->language()->get('bit_rss_title'), \IPS\Member::loggedIn()->language()->get('bit_rss_title') );
