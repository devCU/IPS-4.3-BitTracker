	...
	public function render()
	{
		foreach ( array( 'week' => 'P1W', 'month' => 'P1M', 'year' => 'P1Y', 'all' => NULL ) as $time => $interval )
		{
			...
			$ratings = iterator_to_array( \IPS\Db::i()->select(
				'bitracker_files.file_submitter, AVG(file_rating) as rating, COUNT(file_id) AS files',
				'bitracker_files',
				$where,
			...
			$fileCounts = iterator_to_array( \IPS\Db::i()->select( 'file_submitter, COUNT(*) AS count', 'bitracker_files', $where, NULL, NULL, 'file_submitter' )->setKeyField('file_submitter')->setValueField('count') );
