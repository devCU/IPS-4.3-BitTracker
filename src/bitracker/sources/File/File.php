...
\IPS\Content\MetaData
{
	...
	public function downloadCheck( array $record = NULL, \IPS\Member $member = NULL )
	{
		...
		{
			if ( \IPS\Db::i()->select( 'COUNT(*)', 'bitracker_sessions', $member->member_id ? array( 'dsess_mid=?', $member->member_id ) : array( 'dsess_ip=?', \IPS\Request::i()->ipAddress() ) )->first() >= $restrictions['limit_sim'] )
			{
	...
	protected function createNotification( $extra=NULL )
	{
		...
		if ( $extra === 'update' )
		{
			return new \IPS\Notification( \IPS\Application::load( 'bitracker' ), 'new_file_version', $this, array( $this ) );
		}
		else
		{
	...
	public function syncTopic()
	{
		...
		if ( $this->topicid )
		{
			...
			try
			{
				...
				$content = \IPS\Theme::i()->getTemplate( 'submit', 'bitracker', 'front' )->topic( $this );
