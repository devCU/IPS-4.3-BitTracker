...
class _Bitracker extends \IPS\core\MemberACPProfile\Block
{
	...
	{
		$fileCount = \IPS\Db::i()->select( 'COUNT(*)', 'bitracker_files', array( 'file_submitter=?', $this->member->member_id ) )->first();
		$diskspaceUsed = \IPS\Db::i()->select( 'SUM(file_size)', 'bitracker_files', array( 'file_submitter=?', $this->member->member_id ) )->first();
		...
		return \IPS\Theme::i()->getTemplate( 'stats', 'bitracker' )->information( $this->member, \IPS\Theme::i()->getTemplate( 'global', 'core' )->definitionTable( array(
			...
			'average_filesize_bitracker'		=>
				\IPS\Member::loggedIn()->language()->addToStack('downloads_stat_average', FALSE, array( 'sprintf' => array(
						\IPS\Output\Plugin\Filesize::humanReadableFilesize( \IPS\Db::i()->select( 'AVG(file_size)', 'bitracker_files', array( 'file_submitter=?', $this->member->member_id ) )->first() ),
