...
class _view extends \IPS\Content\Controller
{
	...
	protected function buy()
	{
		...
		$invoice->return_uri = "app=bitracker&module=bitracker&controller=view&id={$this->file->id}";
		$invoice->save();
	...
	protected function download()
	{
		...
		if ( count( $files ) === 1 or ( isset( \IPS\Request::i()->r ) ) )
		{
			...
			/* Time Delay */
			if ( \IPS\Member::loggedIn()->group['bit_wait_period'] )
			{
				if ( isset( \IPS\Request::i()->t ) )
				{
					...
					if ( $_SESSION[ $timerKey ] > ( time() - \IPS\Member::loggedIn()->group['bit_wait_period'] ) )
					{
						\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'view' )->download( $this->file, null, $files, $confirmUrl, count( $files ) > 1, $data['record_id'], ( $_SESSION[ $timerKey ] + \IPS\Member::loggedIn()->group['bit_wait_period'] ) - time() );
						return;
					}
					else
					{
